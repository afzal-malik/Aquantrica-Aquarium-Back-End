<?php
  ...
  $sql =  "insert into accessories_archive select * from accessories "; //space must
  ...
  if($x > 0){
    ...
    $sql_del = "delete from accessories where acc_id=$acc_id";
    ...
    if($y > 0){
      ...
      header("location:delete_accessories_4.php?status=pass");
    }
    else{
      //delete failed so lets send the user to delete_product_4.php
      //with the status failed
      header("location:delete_accessories_4.php?status=fail");
    }//end of if - $y part
  }
  else{
